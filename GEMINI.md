...
## Core Rules
1. **Context Efficiency:** Before starting any task, read `PROGRESS.md`, `HANDOFF.md`, and `TASKS.md`.
2. **Skill Usage:** Always activate the `racing-app-expert` skill at the start of a session using `activate_skill`.
3. **Documentation Sync:** After every task or whenever something is worked on, you **MUST** update `PROGRESS.md` and `TASKS.md`.
